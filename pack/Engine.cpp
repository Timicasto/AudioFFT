...
void Engine::loop(GLFWwindow* window) {
	...
	const float smoothConstantDown = 0.025;
	const float smoothConstantUp = 0.8;

	float lastHigh[1024] = {-1};
	...
	while (!glfwWindowShouldClose(window)) {
		...
		std::vector<double> out = FFT(fft, 1024);
		...
		float vertices[18432];

		for (int rects = 0 ; rects < 1024 ; ++rects) {

			float high = ((float)(lastHigh[rects])) / 320000.0;
			float left = (rects - 512) * (double)0.001953125;
			float right = (rects - 512 + 1) * (double)0.001953125;
			for (int tri = 0 ; tri < 2 ; ++tri) {
				if (tri == 0) {
					vertices[(rects * 2 + tri) * 9] = right;
					vertices[(rects * 2 + tri) * 9 + 1] = high;
					vertices[(rects * 2 + tri) * 9 + 2] = 0.0;

					vertices[(rects * 2 + tri) * 9 + 3] = left;
					vertices[(rects * 2 + tri) * 9 + 4] = high;
					vertices[(rects * 2 + tri) * 9 + 5] = 0.0;

					vertices[(rects * 2 + tri) * 9 + 6] = left;
					vertices[(rects * 2 + tri) * 9 + 7] = -high;
					vertices[(rects * 2 + tri) * 9 + 8] = 0.0;
				} else {
					vertices[(rects * 2 + tri) * 9] = right;
					vertices[(rects * 2 + tri) * 9 + 1] = high;
					vertices[(rects * 2 + tri) * 9 + 2] = 0.0;
					...
					vertices[(rects * 2 + tri) * 9 + 6] = right;
					vertices[(rects * 2 + tri) * 9 + 7] = -high;
					vertices[(rects * 2 + tri) * 9 + 8] = 0.0;
				}
			}
		...
		glBindVertexArray(VAO);
		glDrawArrays(GL_TRIANGLES, 0, 18432);
